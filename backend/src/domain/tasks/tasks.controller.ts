...
export class TasksController {
	...
	async uploadTask(
		...
		const updateTask = await this.tasksService.updateTask(Number(id), updateTaskDto, file.filename);
		return updateTask;
	}
	...
	@ApiResponse({
		...
		schema: {
			example: {
				message: 'Wrong extension type. Accepted file ext are : .png,.jpg,.jpeg',
				error: 'Bad Request',
				statusCode: 400,
			},
		},
	...
	async uploadFile(
		...
		const uploadFileTask = await this.tasksService.uploadFile(Number(id), file.filename);
